...
For brevity reasons, only the permissions related to repositories are listed.

## Repository permissions

Our structure for repository permissions is `(allow or deny):repo/<org>/<repo>/<group>/<action>`.
This allows us to create organization, user or other permissions in a separate top level namespace,
...
These permissions relate to using super wildcards for generalized access.
...
These permissions relate to using wildcards for generalized access.
...
Note that when permissions have multiple levels of variables you may need to wildcard more then one block.
...
User permissions can include variables that relate to actor attempting to do the action.
